...
use crate::{order_manager::order_dispatcher::{OrderManager, Order}, common::{raw_stock::RawStock, enums::TradeType, redis_client::RedisClient, date_parser::{if_first_time_greater_than_second_time, return_only_time_from_datetime}}};

const THRESHOLD_TRADE_END_TIME: Option<NaiveTime> = NaiveTime::from_hms_opt(15, 15, 0);

pub async fn check_for_exit_opportunity(order_manager: &mut OrderManager, stock: RawStock, redis_client: &Mutex<RedisClient>, order_collection: Collection<Order>, shared_order_ledger: &mut Vec<Order>){
    ...
    for (index, order) in shared_order_ledger.iter_mut().enumerate(){
        ...
        if order.symbol == stock.symbol && order.is_trade_open && order.is_trade_executed{
            ...
            if exit_price > 0.0{
                ...
                if updated_order.is_some(){
                    ...
                    order.trade_closed_at = temp_order.trade_closed_at;
                    order.is_profitable_trade = temp_order.is_profitable_trade;
                    println!();
                    println!("#################");
                ...
                let updated_order = order_manager.exit_and_update_order( order, stock.close, redis_client, &order_collection).await;
                // println!("updated order: {:?} for index {}", updated_order, index);
                if updated_order.is_some(){
                    let temp_order = updated_order.unwrap();
                    order.is_profitable_trade = temp_order.is_profitable_trade;
        ...
        } else if !order.is_trade_executed{
            if stock.open >= order.entry_price && order.trade_position_type == TradeType::Long{
                ...
                if updated_order.is_some(){
                    ...
                    order.trade_executed_at = temp_order.trade_executed_at;
                    println!();
                    println!("#################");
